-- Baby in Car - Initial Database Schema
-- Run with: wrangler d1 execute babyincar-db --file=./migrations/001_initial.sql
...
CREATE TABLE IF NOT EXISTS users (
  ...
  password_hash TEXT,
  name TEXT,
  auth_provider TEXT DEFAULT 'email' CHECK (auth_provider IN ('email', 'apple')),
  apple_user_id TEXT,
...
CREATE TABLE IF NOT EXISTS tracks (
  ...
  title TEXT NOT NULL,
  artist TEXT DEFAULT '',
  category TEXT NOT NULL CHECK (category IN ('classical_music', 'fairy_tales', 'white_noise', 'nature_sounds', 'instrumental', 'children_songs', 'podcasts')),
  ...
  audio_source_type TEXT NOT NULL CHECK (audio_source_type IN ('generated', 'recorded', 'text_to_speech')),
  generator_type TEXT,
  stream_url TEXT,
  artwork_url TEXT,
...
CREATE TABLE IF NOT EXISTS playlists (
  ...
  category TEXT CHECK (category IN ('classical_music', 'fairy_tales', 'white_noise', 'nature_sounds', 'instrumental', 'children_songs', 'podcasts', NULL)),
...
CREATE TABLE IF NOT EXISTS playback_events (
  ...
  context TEXT DEFAULT 'home' CHECK (context IN ('home', 'carplay', 'emergency')),
  session_id TEXT,
  created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
  FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS track_effectiveness (
  ...
  context TEXT DEFAULT 'crying' CHECK (context IN ('crying', 'fussy', 'sleepy')),
