...
@Suite("Audio Ducking Integration")
@MainActor
struct AudioDuckingIntegrationTests {

    @Test("Ducking preference defaults to enabled")
    func duckingPreferenceDefaultsToEnabled() {
        // Clear any existing preference
        UserDefaults.standard.removeObject(forKey: "audioEngine.autoDuckExternalAudio")

        // Default should be true (enabled)
        #expect(AudioEngine.autoDuckExternalAudio == true,
               "Auto-duck should default to TRUE for baby safety")
    }

    @Test("Ducking preference can be toggled")
    func duckingPreferenceCanBeToggled() {
        // Enable
        ...
        #expect(AudioEngine.autoDuckExternalAudio == false)
    ...
    @Test("Ducking preference persists to UserDefaults")
    func duckingPreferencePersistsToUserDefaults() {
    ...
    @Test("AudioEngine exposes isDuckingActive state")
    func audioEngineExposesDuckingState() {
        let audioEngine = AudioEngine.shared

        // Ensure preference is enabled
        AudioEngine.autoDuckExternalAudio = true

        // Initial state should be false (not ducking)
        audioEngine.enableDucking(false)
        #expect(audioEngine.isDuckingActive == false)

        // Enable ducking
        audioEngine.enableDucking(true)
        #expect(audioEngine.isDuckingActive == true)
    ...
    @Test("Ducking respects user preference")
    func duckingRespectsUserPreference() {
        let audioEngine = AudioEngine.shared
        ...
        // Try to enable ducking
        audioEngine.enableDucking(true)

        // Should NOT be active because preference is disabled
        #expect(audioEngine.isDuckingActive == false,
               "Ducking should not activate when user preference is disabled")
        ...
        // Should be active
        #expect(audioEngine.isDuckingActive == true,
               "Ducking should activate when user preference is enabled")
