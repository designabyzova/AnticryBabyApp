...
**Acceptance Criteria**:
- [x] **AC-US1-01**: `MemoryMonitor.swift` service tracks app memory usage every 5 seconds
- [x] **AC-US1-02**: Emits warnings at 40MB (80% of 50MB target)
- [x] **AC-US1-03**: Triggers automatic cleanup at 45MB (90% of target)
- [x] **AC-US1-04**: Logs memory breakdown by component (audio, AI, detection)
- [x] **AC-US1-05**: Provides real-time memory stats to debug UI
...
**Acceptance Criteria**:
- [x] **AC-US2-01**: `SmartEmergencyQueue` limits concurrent loaded tracks to 3 (not 10)
- [x] **AC-US2-02**: `AudioEngine` releases buffers for tracks not in active use
- [x] **AC-US2-03**: `DynamicSoundMixer` uses streaming instead of full caching
- [ ] **AC-US2-04**: Total audio buffer usage stays under 15MB
- [x] **AC-US2-05**: Preload next track only, not entire queue
...
**Acceptance Criteria**:
- [x] **AC-US3-01**: `BabyMoodLLMEngine.maxHistorySize` reduced from 500 to 100
- [x] **AC-US3-02**: `AdaptiveLearningEngine.successfulFeatureVectors` reduced from 500 to 100
- [x] **AC-US3-03**: `AdaptiveLearningEngine.maxSessionHistory` reduced from 1000 to 200
- [x] **AC-US3-04**: All AI engines implement LRU eviction for history
...
**Acceptance Criteria**:
- [x] **AC-US5-01**: XCTestCase `MemoryProfilingTests.swift` created
- [x] **AC-US5-02**: Test: 30-min cry detection session stays < 50MB
- [x] **AC-US5-03**: Test: 10-track audio playback stays < 50MB
- [x] **AC-US5-04**: Test: Emergency mode transitions stay < 50MB
- [x] **AC-US5-05**: Test: All AI engines active simultaneously < 50MB
- [x] **AC-US5-06**: Tests fail if memory exceeds 50MB threshold
...
**Acceptance Criteria**:
- [x] **AC-US6-01**: At 40MB warning: Log detailed breakdown
- [x] **AC-US6-02**: At 45MB critical: Trigger automatic cleanup (clear caches, release old buffers)
- [x] **AC-US6-03**: At 48MB emergency: Force aggressive cleanup (stop non-essential services)
- [x] **AC-US6-04**: User sees non-intrusive notification if cleanup triggered
- [x] **AC-US6-05**: Memory usage drops to < 35MB after auto-cleanup
